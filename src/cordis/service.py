...
class Service:
    ...
    async def refresh(self) -> None:
        """Notify consumers after the availability predicate changes."""

        label = self.context.root.reflect.label(self.context, self.name)
        await self.context.root.reflect.notify(self.name, label)
    ...
    def resolve_config(
        ...
        for config in (base, *context.intercepts.get(self.name, ()), head):
            ...
            result.update(cast(Mapping[str, object], raw_mapping))
        return result
